...
| `test_dictionary.py` | The model's own invariants, the BAK/BCH/BCA layouts checked element by element against 004010 as written in the test rather than read from the dictionary, the published dictionary, `/_mock/validate` - and `GeneratedDocumentsAreValid`, which checks every document the mock writes against the dictionary it checks yours against. |
...
| `test_duplicates.py` | A replayed interchange refused rather than fulfilled twice: the TA1 with note code 025, the CONTRL with `0085 = 27`, no 997 or business documents behind a refused envelope, another partner free to use the same number, `--allow-duplicates`, and the archive keeping the replay. |
