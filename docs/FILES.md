...
| `test_exposure.py` | Running where others can reach it: the startup warning when listening beyond loopback without `--auth` or `--deliver-to`, the courier's allowlist refusing a partner URL, a document already bound for a refused host and an asynchronous MDN asked for outside it, and credentials compared as bytes. |
