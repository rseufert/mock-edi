...
| `documents.py` | What the seller decides - which lines to confirm, how many, at what price - and the shipment and invoice that follow. The decision rules and their precedence are documented in its docstring. | Changing the business decisions or the fulfilment documents. |
| `partners.py` | The partner records and the behaviours: who the mock trades with, and how each one misbehaves. | Adding a behaviour or a partner field. |
