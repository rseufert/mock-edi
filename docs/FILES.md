...
| `test_versions.py` | The versions the mock speaks: REF02 at 30 characters in 004010 and 50 in 005010, ST03 only from 005010, a GS08 industry suffix read as its version, an unspoken version refused with AK905 2, a 997 at 004010 without AK103, everything written for GLOBEX passing the 005010 dictionary, and the dictionary served by version. |
...
| `test_exposure.py` | What the mock says and refuses when it is reachable: the startup warning for a non-loopback bind with no `--auth`, `--deliver-to` as an allowlist of hosts the courier may post to, a `Receipt-Delivery-Option` outside it answered with a failure MDN rather than posted to, and credentials compared in constant time. |
...
| `test_clock.py` | One clock and one format: every control-plane timestamp UTC to the second with a `Z`, two stamps sorting the way their moments do, and - with `TZ` set fourteen hours from UTC - a delay due where a caller expects it, nothing released early, and the wire's ISA09 still local. |
